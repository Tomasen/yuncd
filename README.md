...
draft = truehost
title = "Remote API"
description = "API Documentation for crane"
keywords = ["API, crane, rcli, REST,  documentation"]
<![end-metadata]-->
...
| Name | Explanation |
| ---- | ----------- |
| Host   | machine that running docker service hosts
| Job    | sepecified service |
| Strategies | auto-scaling, single host, or mass-deployment |

## Remote API

| method | endpoints       | desc |
| ------ | --------------- | ---- |
<!--
| GET    | /keys/json      | list key pairs    |
| POST   | /keys/(id)      | set a key pair    |
| DELETE | /keys/(id)      | delete a key pair |
-->
host的premission：public 使用的, private to owner 或 group 的
用户 premission 怎么界定
不用 auth? 和 premission 可以么? 不会有安全问题么？

| POST   | /hosts/add      | add a host        |
| POST   | /hosts/(id)/set | set a host        |
| GET    | /hosts/json     | list hosts        |
| DELETE | /hosts/(id)     | delete a host     |
| POST   | /jobs/add       | add a job         |
| POST   | /jobs/(id)/set  | set up a job      |
| GET    | /jobs/json      | list jobs         |
| DELETE | /jobs/(id)      | delete a job      |

### Auth

the auth configuration is handled by JWT
...
-   **id** – specidied the host `id` to set up

Json Parameters:

-   **Addr** – ip and port address
...
-   **name** – Assign the specified name to the host. Must
        match `/?[a-zA-Z0-9_-]+`.

Request Headers:

-   **X-Crane-Auth** – JSON Web Tokens
...
**Example request**:
        ...
        POST /hosts/8dfafdbc3a40/set HTTP/1.1
        {
             "Addr":"172.26.32.9:2375",
             "CACert":"....",
             "Cert":"....",
...
**Example request**:
        ...
        GET /hosts/json?limit=10&before=8dfafdbc3a40 HTTP/1.1
        ...
        [
             {
                     "Id": "8dfafdbc3a40",
                     "Addr":"172.26.32.9:2375",
                     "KeyPair": "2048 MD5:4d:5b:97:19:8c:fe:06:f0:29:e7:f5:96:77:cb:3c:71",
                     "Created": 1367854155
             },
			 {
                     "Id": "8dfafdbc3a41",
...
-   **limit** – Show `limit` last created keys.
-   **since** – Show only keys created since Id.
-   **before** – Show only keys created before Id.
...
Remove the host `id` from the pool

**Example request**:

        DELETE /host/16253994b7c4?t=1 HTTP/1.1
...
-   **t** – 1/True/true or 0/False/false, Remove the jobs
        associated to the host. Default false
...
**Example request**:
        ...
        GET /keys/json?limit=10&before=8dfafdbc3a40 HTTP/1.1
...
**Example response**:
        ...
        [
             {
                     "Id": "8dfafdbc3a40",
                     "FingerPrint": "1024 SHA256:19n6fkdz0qqmowiBy6XEaA87EuG/jgWUr44ZSBhJl6Y",
                     "Created": 1367854155
             },
			 {
                     "Id": "8dfafdbc3a40",
                     "FingerPrint": "2048 MD5:4d:5b:97:19:8c:fe:06:f0:29:e7:f5:96:77:cb:3c:71",
                     "Created": 1367854158
             }
...
-   **CACert** – private key pair

Query Parameters:

     

-   **name** – Assign the specified name to the key pair. Mus
        match `/?[a-zA-Z0-9_-]+`.
