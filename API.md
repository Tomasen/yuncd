...
-->
node的premission：public 使用的, private to owner 或 group 的
用户 premission 怎么界定
不用 auth? 和 premission 可以么? 不会有安全问题么？

| POST   | /nodes/add      | add a node        |
| POST   | /nodes/(id)/set | set a node        |
| GET    | /nodes/json     | list nodes        |
...
-   **id** – specidied the node `id` to set up
...
-   **name** – Assign the specified name to the node. Must
        match `/?[a-zA-Z0-9_-]+`.
...
**Example request**:
        ...
        POST /nodes/8dfafdbc3a40/set HTTP/1.1
        {
             "Addr":"172.26.32.9:2375",
             "CACert":"....",
             "Cert":"....",
...
**Example request**:
        ...
        GET /nodes/json?limit=10&before=8dfafdbc3a40 HTTP/1.1
...
Remove the node `id` from the pool

**Example request**:

        DELETE /nodes/16253994b7c4?t=1 HTTP/1.1
...
-   **t** – 1/True/true or 0/False/false, Remove the jobs
        associated to the node. Default false
